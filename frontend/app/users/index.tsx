...
const UsersListScreen = () => {
  ...
  const fetchBrands = useCallback(async () => {
    try {
      const endpoint =
        currentUser?.role === UserRole.ADMIN ? "/brands/admin" : "/brands";
  ...
  const assignBrandToUser = async (
    ...
    try {
      ...
      const response = await fetch(
        `${getApiUrl()}/brands/${brandId}/assign-user`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const removeUserFromBrand = async (brandId: number, userId: number) => {
    try {
      setProcessing(true);
      const response = await fetch(
        `${getApiUrl()}/brands/${brandId}/remove-user/${userId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          errorData.message || "Failed to remove user from brand",
        );
      }
      ...
      <View style={styles.userInfo}>
        ...
        {item.brandUsers && item.brandUsers.length > 0 && (
          <View style={styles.assignedBrands}>
            <Text style={[styles.smallLabel, { color: secondaryTextColor }]}>
              Assigned Brands:
            </Text>
            <View style={styles.brandChipContainer}>
              {item.brandUsers.map((bu) => (
                <View
                  key={bu.id}
                  style={[styles.brandChip, { backgroundColor: borderColor }]}
                >
                      ...
                      Alert.alert(
                        ...
                        [
                          { text: "Cancel", style: "cancel" },
                          {
                            text: "Remove",
                            onPress: () =>
                              removeUserFromBrand(bu.brandId, item.id),
                            style: "destructive",
                          },
                        ],
                      );
      ...
      <View style={styles.actions}>
        ...
        >
          <Ionicons
            ...
            size={18}
            color={
              item.id === currentUser?.id ? secondaryTextColor : buttonColor
            }
          />
          <Text
            style={[
        ...
        {item.role === UserRole.BRAND_OWNER && (
          ...
          >
            ...
            <Text style={[styles.actionText, { color: buttonColor }]}>
              Assign Brand
            </Text>
          </TouchableOpacity>
        )}
    ...
    >
      ...
      <View style={styles.searchSection}>
        ...
        <ScrollView
          horizontal
          showsHorizontalScrollIndicator={false}
          style={[styles.filters, { marginTop: 8 }]}
        >
          <Text style={[styles.filterLabel, { color: secondaryTextColor }]}>
            Status:
          </Text>
            ...
            >
              <Text
                style={[
                  ...
                  {
                    color:
                      statusFilter === status ? "#fff" : secondaryTextColor,
                  },
                ]}
              >
      ...
      <Modal visible={showBrandModal} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View
            style={[
              styles.modalContent,
              { backgroundColor: cardBackground, height: "70%" },
            ]}
          >
            <Text style={[styles.modalTitle, { color: textColor }]}>
              Assign Brand
            ...
            <ScrollView style={{ flex: 1 }}>
              {brands.map((brand) => (
                ...
                >
                  <View>
                    <Text
                      style={[styles.modalOptionText, { color: textColor }]}
                    >
                      {brand.name}
                    </Text>
                    <Text style={{ fontSize: 12, color: secondaryTextColor }}>
                      Current Owners:{" "}
                      {brand.brandUsers?.filter(
                        (bu) => bu.role === BrandRole.OWNER,
                      ).length || 0}
                    </Text>
                  </View>
                  <Ionicons
