  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!membership) {
      throw new ForbiddenException(
        'Access denied. You do not have permissions for this brand.',
      );
    }
    ...
    if (
      ...
      if (!membership.canManageProducts) {
        throw new ForbiddenException(
          'Access denied. You do not have permission to manage products for this brand.',
        );
      }
    }
    ...
    if (url.match(/\/brands\/\d+$/) && method === 'PUT') {
      if (!membership.canEditBrandProfile) {
        throw new ForbiddenException(
          "Access denied. You do not have permission to edit this brand's profile.",
        );
      }
    }
